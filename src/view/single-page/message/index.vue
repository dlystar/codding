  ...
  <Card shadow>
    <div>
      ...
      <div class="message-page-con message-list-con">
        ...
        >
          <MenuItem v-for="item in messageList" :name="item.msgId" :key="`msg_${item.msgId}`" :class="item.class">
            <div>
...
export default {
  ...
  methods: {
    ...
    handleView (msg_id) {
      let index = this[listDic[this.currentMessageType]].findIndex(_ => _.msgId === msg_id)
      this[listDic[this.currentMessageType]][index].class = 'top20'
      setTimeout(()=> {
        this[listDic[this.currentMessageType]][index].class = ''
      },200)
