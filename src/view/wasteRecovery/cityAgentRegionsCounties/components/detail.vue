    ...
    <div class="baseInfo">
        <Form :model="detail" ref="detail" :rules="rules" :label-width="120">
            ...
            <FormItem label="市级代理人：" prop="cityAgentName">
                <Input v-model="detail.cityAgentName" :disabled="noEdit || isDisabled" readonly>
    ...
    export default {
        ...
        mounted(){
            ...
            this.axios.request({
                ...
                url: '/adminCityAgent/findCityAgentByUserId',
                method: 'get'
            }).then(res =>{
                this.detail.cityAgentName = res.data.rows.userName
                this.detail.cityAgentId = res.data.rows.id
                this.detail.city = res.data.rows.city
                this.matchCity()
                this.$forceUpdate()
            }).catch(err =>{
                console.log('网络异常')
